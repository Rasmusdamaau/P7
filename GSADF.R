...
{
  if (base::is.null(x)) {
    stock_data <- tidyquant::tq_get(ticker, from = date_from, 
                                    to = date_to) %>% dplyr::select(date, adjusted) %>% 
      stats::na.omit()
  ...
  while (window_size < (nrow_x - step_length)) {
    ...
    for (i in 0:max_i) {
      x_window <- x[(step_length * i):((step_length * 
                                          i) + window_size)]
      plot_data <- tibble::tibble(t = base::seq_along(x_window), 
                                  x_window)
      if (drift == F & trend == F) {
      ...
      result[k_ind, "estimate"] <- coef_model[t_val_model_nr, 
                                              1]
      result[k_ind, "std_error"] <- coef_model[t_val_model_nr, 
                                               2]
      result[k_ind, "t_value"] <- coef_model[t_val_model_nr, 
                                             3]
  ...
  result <- result %>% dplyr::rowwise() %>% dplyr::mutate(p_val = p_val_fun(t_val = t_value, 
                                                                            interval_length = interval_length, distribution_tibble = distribution_tibble))
  result_list <- base::list(stock = tibble::tibble(date = date_x, 
                                                   price = x), result = result)
