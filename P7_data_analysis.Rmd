...
GSADF_fun <- function(x, min_window, step_length, max_window=(length(x)-5)) {
  n_row_x <- length(x)
  ...
  while (j <= max_window){
    k_ind <- 1
    max_i <- floor((n_row_x - j) / step_length)
    for (i in 0:(max_i)) {
    adf_list[[k_ind]] <- suppressWarnings(adf.test(x = x[(step_length*(i)):((step_length*(i)) + j)],
                              alternative = "stationary",
                              k = 0))
    result[k_ind+k_end, "t_value"] <- adf_list[[k_ind]][["statistic"]][["Dickey-Fuller"]]
    result[k_ind+k_end, "start_day"] <- step_length*(i)
    result[k_ind+k_end, "end_day"] <- ((step_length*(i)) + j)
    result[k_ind+k_end, "interval_length"] <- j
    k_ind <- k_ind + 1
    }
    j <- j + 10
    k_end <- k_ind-1+k_end
