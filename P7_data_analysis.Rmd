...
p_val_fun <- function(t_val, interval_length, ditribution_tibble) {
  
  sum(t_val > ditribution_tibble[,
                                 paste("T_val_", interval_length, sep = "")]) /
    nrow(ditribution_tibble)
}

GSADF_fun <- function(ticker = "btc-usd", x = NULL, min_window = 20,
  ...
  distri_tibbles <- c("DF_distribution.RData",
 ...
 ditribution_tibble <- if (t_val_model_nr == 1) {
  df_distri
  ...
  result <- result %>%
    ...
    mutate(p_val = p_val_fun(
      ...
      ditribution_tibble = ditribution_tibble
    ))
    result_list <- list(stock = tibble(date = date_x,
                                       price = x),
                        result = result)
  return(result_list)
}
...
GSADF_fun_result <- GSADF_fun(
  ...
  date_from = "2019-01-01", date_to = Sys.Date(), drift = T, trend = T
)
...
ggplot(data = GSADF_fun_result$stock, aes(x = seq_along(date), y = price)) +
  geom_line() +
  geom_rect(data = plot_data, aes(xmin = start_day, xmax = end_day),
