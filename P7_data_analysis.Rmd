...
GSADF_fun <- function(ticker = "btc-usd", x = NULL, min_window,
                      step_length, window_increase,
                      date_from = "1900-01-01", date_to = Sys.Date(),
                      drift = F, trend = F) {
  if (is.null(x)) {
    stock_data <- tq_get(ticker, from = date_from, to = date_to)
    x <- na.omit(stock_data$adjusted)
  }
  nrow_x <- length(x)
