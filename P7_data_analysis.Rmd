...
Call_MA <- function(Stock,y,from,to){
  
  x <- tq_get(Stock, get = "stock.prices", from = from, to = to)
  ...
  ggplot(Data_nona[-(1:y-1),], aes(x = date)) +
  geom_line(aes(y=MA), colour = "red")+
  geom_line(aes(y=adjusted), colour = "blue")
}

Call_MA("GC=F",200,"2019-01-01","2020-09-08")
