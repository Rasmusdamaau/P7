...
combined_count <- function(u, d, d_t, p_restrict) {
  ...
  combined_df <- data.frame(
    x = seq_along(combined_u),
    u = combined_u,
    d = combined_d,
    d_t = combined_d_t
  )
  return(combined_df %>% pivot_longer(-1))
}

plot_data_combined <- combined_count(u, d, d_t, 0.95)


ggplot(data = plot_data_combined, aes(x = x, y = value, color = name)) +
  geom_line(size = 1)
